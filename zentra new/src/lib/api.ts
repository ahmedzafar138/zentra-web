    ...
    Object.entries(weeklyPlan).map(([dayKey, dayPlan]) => [
      ...
      Object.fromEntries(
        Object.entries(dayPlan).map(([mealKey, meal]) => [
          mealKey,
          { food: meal.food, portion: meal.portion },
        ]),
      ),
    ]),
  );
  return postJson<{ data: ShoppingList }>(
    MEAL_API_BASE_URL,
...
export function parseMealPlan(value: unknown): WeeklyMealPlan {
  const raw =
    typeof value === "string"
      ? JSON.parse(value.replace(/```json\s*/i, "").replace(/```/g, "").trim())
      : value;
  const candidate =
    (raw as { meal_plan?: unknown; plan?: unknown })?.meal_plan ??
    (raw as { plan?: unknown })?.plan ??
    raw;
  const firstDay = (candidate as { day1?: unknown })?.day1
    ? candidate
    : { day1: candidate };
...
export function loadBicepCurlModel() {
  return postJson<Record<string, unknown>>(MODEL_GATEWAY_API_BASE_URL, "/api/v1/bicep-curl/load");
}

export function loadSquatModel() {
  return postJson<Record<string, unknown>>(MODEL_GATEWAY_API_BASE_URL, "/api/v1/squats/load");
}

export function loadDeadliftModel() {
  return postJson<Record<string, unknown>>(MODEL_GATEWAY_API_BASE_URL, "/api/v1/deadlifts/load");
}

export function loadPlankModel() {
  return postJson<Record<string, unknown>>(MODEL_GATEWAY_API_BASE_URL, "/api/v1/planks/load");
}

export function loadPushupModel() {
  return postJson<Record<string, unknown>>(MODEL_GATEWAY_API_BASE_URL, "/api/v1/pushups/load");
}
